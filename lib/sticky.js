    ...
    module.directive('sticky', ['$window', '$timeout', function ($window, $timeout) {
            return {
                ...
                link: function linkFn($scope, $elem, $attrs) {
                    ...
                    var stickyTriggerOffset = $attrs.stickTriggerOffset;
                    ...
                    var checkIfShouldStick = function () {
                        ...
                        // if we have a stickyTriggerOffset, its an element id
                        // get that element, and then use that's top as the offset
                        var offsetAmount;

                        if (stickyTriggerOffset) {
                            var element = document.getElementById(stickyTriggerOffset)
                            offsetAmount = element.offsetTop;
                        }
                        ...
                        if (anchor === 'top') {
                            if (confine === true) {
                                ...
                                shouldStick = scrollbarPosition > stickyLine + offsetAmount;
                            }
                        } else {
                    ...
                    var onChange = function (newVal, oldVal) {
                        if (( newVal !== oldVal || typeof stickyLine === 'undefined' ) &&
                            ...
                            var elementsDistanceFromTop = elementsOffsetFromTop($elem[0]) + stickyTriggerOffset;
